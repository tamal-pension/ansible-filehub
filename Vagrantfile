...
MAIN_SH_ARGS = <<MARKER
-e "playbook_name=ansible-filehub discord_message_owner_name=#{Etc.getpwuid(Process.uid).name} environment_id=pension-stg.local" --tags "installation,configuration"
MARKER
Vagrant.configure("2") do |config|
  ...
  config.vm.provider :aws do |aws, override|
    ...
    aws.tags = {
        Name: "filehub-test-#{Etc.getpwuid(Process.uid).name}",
        private_dns: "filehub-test-#{Etc.getpwuid(Process.uid).name}"
      }
  end
end